...
# Use override=True so a newly saved key is picked up on the next rerun.
...
with st.sidebar:
    ...
    api_key_input = st.text_input(
        "Google API key",
        type="password",
        value=st.session_state.get("google_api_key_input", os.getenv("GOOGLE_API_KEY", "")),
        help="Enter a key here to override the .env value for this session."
    )
    st.session_state.google_api_key_input = api_key_input

    current_api_key = api_key_input or os.getenv("GOOGLE_API_KEY", "")
    st.caption("Active key: " + ("set" if current_api_key else "missing"))
...
current_api_key = st.session_state.get("google_api_key_input", os.getenv("GOOGLE_API_KEY", "")) or os.getenv("GOOGLE_API_KEY", "")

if "agent" not in st.session_state or st.session_state.get("agent_api_key") != current_api_key:
    # Rebuild the agent whenever the active API key changes.
    try:
        if not current_api_key:
            st.error("No Google API key is available. Enter one in the sidebar or set GOOGLE_API_KEY in .env.")
            st.stop()
